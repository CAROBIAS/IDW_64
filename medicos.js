...
function getImagenObraSocial(id) {
    ...
    const os = obrasSociales.find(o => o.id === id);
    if (!os) return null;
    return os.imagen || `https://ui-avatars.com/api/?name=${encodeURIComponent(os.nombre)}&background=random&size=80`;
}
...
function cargarObrasSociales() {
    ...
    if (obrasSociales.length === 0) {
        container.innerHTML = '<p class="text-muted">No hay obras sociales disponibles</p>';
        return;
    }
    
    obrasSociales.forEach(os => {
        const imagenSrc = os.imagen || `https://ui-avatars.com/api/?name=${encodeURIComponent(os.nombre)}&background=random&size=80`;
...
// Cargar opciones de especialidades con checkboxes
function cargarEspecialidades() {
    ...
    if (especialidades.length === 0) {
        container.innerHTML = '<p class="text-muted">No hay especialidades disponibles</p>';
        return;
    }
    ...
    especialidades.forEach(esp => {
        ...
        div.innerHTML = `
            <input class="form-check-input especialidad-check" type="checkbox" value="${esp.id}" id="esp_${esp.id}">
...
function openCreateModal() {
    ...
    // Limpiar selecciones
    document.querySelectorAll('.especialidad-check').forEach(cb => cb.checked = false);
    document.querySelectorAll('.obra-social-item').forEach(item => item.classList.remove('selected'));
...
function viewMedico(id) {
    ...
    const especialidadesTexto = especialidadesActivas.length > 0 
    ...
    if (medico.obrasSociales && medico.obrasSociales.length > 0) {
        ...
        if (obrasSocialesActivas.length > 0) {
            ...
            obrasSocialesHtml += '<p class="text-muted">Sin obras sociales activas</p>';
        }
    } else {
    ...
    const content = `
        <div class="row">
            <div class="col-md-3 text-center mb-3">
                ...
                ` : `
                    <img src="https://ui-avatars.com/api/?name=${encodeURIComponent(medico.nombre + ' ' + medico.apellido)}&background=random&size=150" alt="Avatar" class="img-fluid rounded-circle" style="width: 150px; height: 150px;">
                `}
            </div>
            <div class="col-md-9">
                ...
                <p><strong>Valor Consulta:</strong> $${parseFloat(medico.valorConsulta).toFixed(2)}</p>
                <p><strong>Teléfono:</strong> ${medico.telefono}</p>
                <p><strong>Email:</strong> ${medico.email}</p>
                <p><strong>Estado:</strong> ${medico.activo ? '<span class="badge bg-success">Activo</span>' : '<span class="badge bg-secondary">Inactivo</span>'}</p>
            </div>
        </div>
        <div class="row mt-3">
            <div class="col-12">
...
function editMedico(id) {
    ...
    setTimeout(() => {
        document.querySelectorAll('.especialidad-check').forEach(cb => {
            cb.checked = medico.especialidades && medico.especialidades.includes(parseInt(cb.value));
        });
    }, 50);
    ...
    setTimeout(() => {
        document.querySelectorAll('.obra-social-check').forEach(cb => {
            const isSelected = medico.obrasSociales && medico.obrasSociales.includes(parseInt(cb.value));
    ...
    if (medico.fotografia) {
        document.getElementById('previewImg').src = medico.fotografia;
...
async function guardarMedico() {
    ...
    const especialidadCheckboxes = document.querySelectorAll('.especialidad-check:checked');
    const especialidadesSeleccionadas = Array.from(especialidadCheckboxes).map(cb => parseInt(cb.value));
    
    const id = document.getElementById('medicoId').value;
    const activo = document.getElementById('activo').checked;
    
    if (activo && especialidadesSeleccionadas.length === 0) {
        if (confirm('No ha seleccionado especialidades. ¿Desea asignar "Medicina General" por defecto?')) {
            especialidadesSeleccionadas.push(4);
        } else {
            alert('Debe seleccionar al menos una especialidad para activar al médico.');
            return;
        }
    }
    
    const checkboxes = document.querySelectorAll('.obra-social-check:checked');
    const obrasSocialesSeleccionadas = Array.from(checkboxes).map(cb => parseInt(cb.value));
    ...
    alert('Médico guardado correctamente' + (estaraActivo !== activo ? ' (Desactivado por falta de especialidades)' : ''));
}
...
function deleteMedico(id) {
    ...
    if (!confirm(`¿Está seguro de desactivar a ${medico.nombre} ${medico.apellido}?`)) return;
